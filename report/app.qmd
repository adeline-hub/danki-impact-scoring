---
title: "Danki Impact Scoring App"
format:
  html:
    page-layout: full
    toc: false
...
<title>Danki Impact Scoring App</title>
...
:root{
  --accent:#00B4D8;--social:#E040FB;--grey:#737373;
  --bg:#F8F9FA;--surface:#FFFFFF;--border:#E5E5E5;--border-strong:#D0D0D0;
  --ink:#0D1B2A;--ink-2:#1B2A3D;--ink-3:#6C757D;
  --accent-d:#0096B7;--social-d:#CC00CC;
  --dg:#00C853;--green:#4CAF50;--amber:#FFB300;--red:#FF5252;
...
.top-logo{font-family:'Inter',sans-serif;font-size:1.1rem;font-weight:700;color:var(--ink);letter-spacing:-.01em}
...
.app-header h1{font-family:'Inter',sans-serif;font-size:clamp(2rem,4vw,2.8rem);font-weight:700;letter-spacing:-.02em;line-height:1.15;color:var(--ink);margin-bottom:.9rem}
.app-header h1 em{font-style:normal;font-weight:400;color:var(--accent)}
...
.card.card-social::before{background:var(--social)}
...
.field input:focus,.field select:focus{outline:none;border-color:var(--accent-d);box-shadow:0 0 0 3px rgba(0,180,216,.1)}
.field select option{background:var(--surface)}
.field input[type="range"]{padding:0;height:3px;background:var(--border);border:none;cursor:pointer;accent-color:var(--accent-d);border-radius:100px}
...
.section-label{font-family:'DM Mono',monospace;font-size:.6rem;letter-spacing:.16em;text-transform:uppercase;color:var(--accent-d);margin-bottom:.75rem;display:block}
...
.score-number{font-family:'Inter',sans-serif;font-size:4.5rem;font-weight:700;line-height:1;letter-spacing:-.03em}
...
.veto-banner{background:rgba(224,64,251,.06);border:1px solid rgba(224,64,251,.3);border-radius:8px;padding:.75rem 1rem;margin-top:1rem;font-size:.78rem;color:var(--social-d);line-height:1.6}
.veto-banner strong{color:var(--social)}
...
.reg-value{font-family:'Inter',sans-serif;font-size:1.1rem;font-weight:600;color:var(--ink)}
...
.tcfd-low {background:rgba(0,200,83,.1);color:var(--dg)}
.tcfd-med {background:rgba(255,179,0,.1);color:#8a5500}
.tcfd-high{background:rgba(255,82,82,.1);color:var(--red)}
...
.flag-warn{background:rgba(255,179,0,.08);border:1px solid rgba(255,179,0,.3);color:#7a4f00}
.flag-warn strong{color:var(--amber)}
.flag-strength{background:rgba(76,175,80,.08);border:1px solid rgba(76,175,80,.3);color:#1a4d2e}
...
.sfdr-rec.art9{background:rgba(0,200,83,.06);border-left:4px solid var(--dg)}
.sfdr-rec.art8{background:rgba(76,175,80,.06);border-left:4px solid var(--green)}
...
.sfdr-art-name{font-family:'Inter',sans-serif;font-size:1.1rem;font-weight:600;color:var(--ink)}
...
<nav class="top-bar">
  <div class="top-bar-left">
    <span class="top-logo">DANKI <span>IMPACT</span></span>
    <div class="top-divider"></div>
    <span class="top-label">Impact Scoring &middot; v1.0</span>
  </div>
  <div class="sdg-strip">
...
<div class="app-wrap">
  ...
  <header class="app-header">
    <span class="header-eyebrow">Danki Impact Scoring</span>
    <h1>Project Impact<br/><em>Assessment</em></h1>
    <p>Enter project parameters to receive a composite Danki score, regulatory
  ...
  <div class="input-grid">
    ...
    <div class="card card-social">
      ...
      <div style="font-size:.75rem;color:var(--ink-3);line-height:1.7;background:var(--bg);border-radius:8px;padding:.8rem 1rem;border:1px solid var(--border)">
        Leave at default to use sector and country benchmarks from the Danki reference database.
        Override only when you have audited project-specific data.
      </div>
      <button class="calc-btn" onclick="calculate()">Run Danki Scoring</button>
    </div>
  </div>
  ...
  <div id="resultsWrap" class="results-hidden">
    ...
    <div class="results-grid" style="margin-bottom:1.25rem">
      ...
      <div class="card" id="scoreCard">
        <div class="card-title">03 &mdash; Danki Composite Score</div>
        ...
        <div style="display:flex;gap:.6rem;align-items:center;margin-top:.5rem">
          ...
          <span id="mifidScore" style="font-family:'Inter',sans-serif;font-size:1.1rem;font-weight:600;color:var(--ink)">—</span>
  ...
  <footer class="app-footer">
    <div class="footer-left">Danki Impact Scoring v1.0 &middot; Danki Studio &middot; Nambona YANGUERE</div>
...
function computeDanki(dims){return Object.keys(WEIGHTS).reduce((s,k)=>s+WEIGHTS[k]*dims[k],0);}
...
function calcSFDR(danki,taxAligned,pai,gov){
  if(taxAligned&&danki>=72&&pai>=70)return"Article 9";
  if(danki>=50&&pai>=50)return"Article 8";
  return"Article 6";
}
...
function calcMiFID(danki,sfdr,phys,trans){
  let b=danki/10;
...
function calcTaxonomy(sector,ghg,vuln,danki){
  const[elig]=SECTOR[sector]||[false];
  if(!elig)return[false,false,false,0];
  const sc=clamp((1-ghg)*.6+(danki/100)*.4,0,1);
...
function calculate(){
  ...
  const rawDanki = computeDanki(dims);
  const vetoTriggered = dims.gender<VETO_THRESHOLD||dims.social<VETO_THRESHOLD;
  let bandStr;
  if(rawDanki>=75)bandStr="Dark Green";
  else if(rawDanki>=58)bandStr="Green";
  else if(rawDanki>=40)bandStr="Amber";
  else bandStr="Red";
  if(vetoTriggered&&(bandStr==="Dark Green"||bandStr==="Green"))bandStr="Amber";

  const[taxE,taxA,dnsh,sc]=calcTaxonomy(sector,ghg,vuln,rawDanki);
  const pai=calcPAI(ghg,gf,cpi,vuln,dims.governance);
  const sfdr=calcSFDR(rawDanki,taxA,pai,dims.governance);
  const[phys,trans]=calcTCFD(vuln,ghg,sector);
  const[ms,mp]=calcMiFID(rawDanki,sfdr,phys,trans);

  lastResult={
    projName,country,sector,ac,eur,
    danki:Math.round(rawDanki*10)/10,band:bandStr,dims,
...
// ── RENDER ────────────────────────────────────────────────────────
const BAND_COLORS={"Dark Green":"#00C853","Green":"#4CAF50","Amber":"#FFB300","Red":"#FF5252"};
...
const DIM_COLORS={"gender":"#E040FB","social":"#CC00CC","governance":"#AB47BC","climate":"#00B4D8","pollution":"#0096B7","water":"#26BDE2","territory":"#FFB300","innovation":"#6C757D"};

function renderResults(r){
  // Score hero
  const sn=document.getElementById('scoreNumber');
  sn.textContent=r.danki.toFixed(1);
  ...
  if(r.veto){
    ...
    msg+='Band capped at Amber — social foundation failure is disqualifying under Danki methodology.';
  ...
  DIM_ORDER.forEach(k=>{
    const v=r.dims[k];
    const color=v<30?'#FF5252':(v<50?'#FFB300':DIM_COLORS[k]);
...
function drawRadar(dims){
  ...
  [25,50,75,100].forEach(pct=>{
    ...
    ctx.stroke();
    ctx.fillStyle='#aaa';ctx.font='9px DM Mono,monospace';
  ...
  ctx.closePath();
  ctx.fillStyle='rgba(0,180,216,.12)';ctx.fill();
  ctx.strokeStyle='#00B4D8';ctx.lineWidth=2;ctx.stroke();

  // Social overlay for social dims
  ...
  ctx.strokeStyle='rgba(224,64,251,.4)';ctx.lineWidth=1.5;ctx.stroke();
  ...
  keys.forEach((k,i)=>{
    ...
    ctx.fillStyle=DIM_COLORS[k];
    ctx.fill();
    const lx=cx+Math.cos(a)*(r+22),ly=cy+Math.sin(a)*(r+22);
    ctx.fillStyle='#0D1B2A';ctx.font='bold 10px Inter,sans-serif';ctx.textAlign='center';
...
async function downloadPDF(){
  ...
  const DARK=[13,27,42],ACCENT=[0,180,216],SOCIAL=[224,64,251],GREY=[108,117,125];
  const WHITE=[248,249,250],INK=[13,27,42],DG=[0,200,83],GREEN=[76,175,80];
  const AMBER=[255,179,0],RED=[255,82,82];
  ...
  else{doc.setFillColor(...ACCENT);doc.circle(M+5,24,5,'F');}
  sf('bold',20,WHITE);doc.text('DANKI',logoData?M+30:M+14,22);
  sf('normal',9,GREY); doc.text('Impact Scoring — Project Due Diligence Report',logoData?M+30:M+14,30);
  ...
  sf('bold',8,ACCENT);   doc.text('Danki Studio',W-M,22,{align:'right'});
  sf('normal',7,GREY); doc.text('Nambona YANGUERE',W-M,29,{align:'right'});
  y=56;

  // ── COMPOSITE SCORE ──
  sf('bold',9,ACCENT);doc.text('DANKI COMPOSITE SCORE',M,y);
  doc.setFillColor(...ACCENT);doc.rect(M,y+1.5,36,.5,'F');y+=8;
  const bc=BAND_CLR[r.band]||GREY;
  doc.setFillColor(...bc);rr(M,y,cW,28,3,'F');
  sf('bold',28,WHITE);doc.text(r.danki.toFixed(1),M+10,y+20);
  ...
  // ── DIMENSIONS ──
  sf('bold',9,ACCENT);doc.text('IMPACT DIMENSIONS',M,y);
  doc.setFillColor(...ACCENT);doc.rect(M,y+1.5,30,.5,'F');y+=8;
  DIM_ORDER.forEach((k,i)=>{
    const ry=y+i*8,val=r.dims[k],bw=(cW-60)*(val/100);
    const c=val<30?RED:(val<50?AMBER:(k==='gender'||k==='social'||k==='governance'?[224,64,251]:[0,180,216]));
  ...
  // ── REGULATORY ──
  sf('bold',9,ACCENT);doc.text('REGULATORY FRAMEWORK',M,y);
  doc.setFillColor(...ACCENT);doc.rect(M,y+1.5,36,.5,'F');y+=8;
  const regs=[
  ...
  if(strengths.length){
    if(y>H-60){doc.addPage();doc.setFillColor(248,249,250);doc.rect(0,0,W,H,'F');y=M;}
  ...
  try{
    ...
    if(y+ih+20>H-15){doc.addPage();doc.setFillColor(248,249,250);doc.rect(0,0,W,H,'F');y=M;}
    sf('bold',9,ACCENT);doc.text('SCORE CARD SNAPSHOT',M,y);y+=6;
  ...
  for(let p=1;p<=np;p++){
    ...
    doc.setFillColor(...ACCENT); doc.rect(0,H-12,W,1,'F');
    sf('normal',6,GREY);
    doc.text('Danki Impact Scoring v1.0 — Project Due Diligence Report — Danki Studio',M,H-5);
  ...
  const fname='Danki-'+r.projName.replace(/\s+/g,'-')+'-'+new Date().toISOString().slice(0,10)+'.pdf';
  doc.save(fname);
  btn.disabled=false;
